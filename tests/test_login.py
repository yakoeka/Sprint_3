...
def test_login_by_velvet_button_on_the_main_page(driver):
    driver.get('https://stellarburgers.nomoreparties.site/register')

    conftest.register_user_positive(driver)

    driver.get('https://stellarburgers.nomoreparties.site/')
    WebDriverWait(driver, 3).until(expected_conditions.element_to_be_clickable((By.XPATH, main_page.loginVelvetBtn)))
    driver.find_element(By.XPATH, main_page.loginVelvetBtn).click()
...
def test_login_by_button_in_register_form(driver):
    driver.get('https://stellarburgers.nomoreparties.site/register')
